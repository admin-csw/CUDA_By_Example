...
// 1D texture declarations (different from 2D texture version)
texture<float> textConstSrc;
...
__global__ void copy_const_kernel(float* iptr) {
    ...
    float c = tex1Dfetch(textConstSrc, offset);
...
__global__ void blend_kernel(float* dst, bool dstOut) {
    ...
    if (dstOut) {
        ...
        t = tex1Dfetch(textOut, top);
        l = tex1Dfetch(textOut, left);
        c = tex1Dfetch(textOut, offset);
        r = tex1Dfetch(textOut, right);
        b = tex1Dfetch(textOut, bottom);
    }
...
void animate_heat_1d_texture(DataBlock* d) {
    ...
    printf("Generating Heat Animation with 1D Texture Memory (%d frames)...\n",
           FRAMES);

    // since texture is global and bound, we have to use a flag to
    // select which is in/out per iteration
    volatile bool dstOut = true;

    for (int frame = 0; frame < FRAMES; frame++) {
        // Run simulation steps for this frame (90 iterations per frame)
        for (int i = 0; i < 90; i++) {
            float* in;
            float* out;
            if (dstOut) {
        ...
        // Save frame
        char filename[64];
        sprintf(filename, "heat_1d_texture_frame_%03d.ppm", frame);
    ...
    printf("\n1D Texture Memory Animation Generation Complete!\n");
    printf("Total time: %3.1f ms\n", elapsedTime);
    printf("Time per frame: %3.1f ms\n", elapsedTime / FRAMES);
    printf(
        "Generated %d frames: heat_1d_texture_frame_000.ppm to "
        "heat_1d_texture_frame_%03d.ppm\n",
        FRAMES, FRAMES - 1);
...
int main(void) {
    ...
    // Setup 1D texture memory (key difference from 2D texture version)
    HANDLE_ERROR(
        cudaBindTexture(NULL, textConstSrc, data.dev_constSrc, imageSize));
    HANDLE_ERROR(cudaBindTexture(NULL, textIn, data.dev_inSrc, imageSize));
    HANDLE_ERROR(cudaBindTexture(NULL, textOut, data.dev_outSrc, imageSize));
    ...
    printf("CUDA Heat Animation with 1D Texture Memory (Chapter 7)\n");
    printf("Grid Size: %dx%d\n", DIM, DIM);
    printf("Animation Frames: %d\n", FRAMES);
    printf("Speed: %f\n", SPEED);
    printf("Max Temp: %f\n", MAX_TEMP);
    printf("Min Temp: %f\n", MIN_TEMP);
    printf("Using GPU 1D Texture Memory (tex1Dfetch)\n");
    printf("\n");

    // Generate animation frames
    animate_heat_1d_texture(&data);

    printf("\nTo create GIF animation, use:\n");
    printf(
        "convert heat_1d_texture_frame_*.ppm -delay 10 "
        "heat_1d_texture_animation.gif\n");

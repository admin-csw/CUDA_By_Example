...
const int blocksPerGrid =
    imin(32, (N + threadsPerBlocks - 1) / threadsPerBlocks);

__global__ void dot(int size, float* a, float* b, float* c) {
    __shared__ float cache[threadsPerBlocks];
...
float cuda_host_alloc_test(int size) {
    ...
    HANDLE_ERROR(cudaHostGetDevicePointer((void**)&dev_a, (void*)a, 0));
    HANDLE_ERROR(cudaHostGetDevicePointer((void**)&dev_b, (void*)b, 0));
    HANDLE_ERROR(
        cudaHostGetDevicePointer((void**)&dev_partial_c, (void*)partial_c, 0));

    HANDLE_ERROR(cudaEventRecord(start, 0));

    dot<<<blocksPerGrid, threadsPerBlocks>>>(size, dev_a, dev_b, dev_partial_c);
...
int main(void) {
    ...
    printf("Time taken with cudaMMalloc: %3.1f ms\n", elapsedTime);

    elapsedTime = cuda_host_alloc_test(N);

    printf("Time taken with cudaHostAlloc: %3.1f ms\n", elapsedTime);
}
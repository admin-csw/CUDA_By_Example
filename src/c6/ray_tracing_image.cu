...
void save_ppm(unsigned char *bitmap, int width, int height,
    ...
    fclose(file);
    printf("Ray traced image saved to %s\n", filename);
}
...
int main(void) {
    ...
    HANDLE_ERROR(cudaMemcpy(s, temp_s, SPHERES * sizeof(Sphere),
                            cudaMemcpyHostToDevice));

    free(temp_s);

    printf("Generating GPU Ray Traced Scene (%dx%d)...\n", DIM, DIM);
    printf("Spheres: %d\n", SPHERES);
    ...
    save_ppm(bitmap, DIM, DIM, "ray_traced_spheres.ppm");

    printf("Ray tracing complete!\n");
    printf("Generated scene with %d randomly placed spheres.\n", SPHERES);
